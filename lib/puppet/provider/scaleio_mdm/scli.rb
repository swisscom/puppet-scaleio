...
Puppet::Type.type(:scaleio_mdm).provide(:scaleio_mdm) do
  ...
  def create
    scli('--add_standby_mdm', '--new_mdm_ip', resource[:ips].join(','), '--new_mdm_name', resource[:name], '--mdm_role', resource[:is_tiebreaker] ? 'tb' : 'manager')
